...
def board_xyz_to_paper_xy(board_xyz: np.ndarray) -> np.ndarray:
    """
    OpenCV ChArUco board coordinates are in the board plane.
    For the OpenCV 4.5.x convention used by this project, the board XY origin
    is at the physical bottom-left. X goes right and Y goes upward.

    Physical paper frame here:
      TL=(-116,-92), TR=(109,-92), BR=(109,83), BL=(-116,83)
    hence paper +X goes right and paper +Y goes downward.

    Therefore:
      paper_x = BL_x + board_x
      paper_y = BL_y - board_y
    """
    bx = float(board_xyz[0])
    by = float(board_xyz[1])

    px = float(PAPER_BL[0] + bx)
    py = float(PAPER_BL[1] - by)
