...
class Collector(Node):
    def __init__(self, args):
        ...
        print("=" * 92, flush=True)
        print("E4 FORMAL CHARUCO 12-SPREAD COLLECTOR V3", flush=True)
        print("=" * 92, flush=True)
    ...
    def detect(self, bgr):
        ...
        # Do NOT feed camera intrinsics into interpolation.
        # Keep common image feature extraction independent of Affine/H/PnP.
        ...
        corners = cc.reshape(-1, 2).astype(np.float32)

        # Explicit subpixel refinement.
        h, w = gray.shape[:2]
        safe = (
            (corners[:, 0] >= 6)
            & (corners[:, 0] < w - 6)
...
def parse_args():
    ...
    p.add_argument(
        "--out-root",
        default="/mnt/c/Users/ASUS/Desktop/paper/E4/E4_charuco_calibration_v3",
    )
    return p.parse_args()
