...
"""
E4-A FORMAL HOVER ENDPOINT EXPERIMENT — V2
=============================================
...
Hover endpoint:
  ...
  MEASURE  = registration-predicted surface +10 mm
  same XY during vertical descent
  robot HOLDS at MEASURE pose while the user reads/enters paper-frame XY
  ONLY AFTER the coordinate is saved does the robot rise and return to start

GROUND TRUTH / OUTCOMES
-----------------------
GT = known physical ChArUco corner coordinate in paper_frame.

For each trial, the robot descends to a fixed hover measurement pose and
STAYS THERE. While it is still hovering, the user reads the pointer's actual
paper-frame XY and enters:
    x,y
Example:
    -65.5,32.0

Only after the measurement is confirmed does the robot rise and return.
...
3) measured hover E2E error [PRIMARY]:
      manually measured pointer XY - GT paper XY
4) downstream residual:
      manually measured pointer XY - predicted paper XY
...
- Before MEASURE hover, the script checks the HIGH T105 pose.
        ...
        for target_order_index, tid in enumerate(order, start=1):
            ...
            for method_order_index, method in enumerate(
                ...
                rows.append({
                    ...
                    "robot_measure_hover_z_mm": rz + MEASURE_HOVER_OFFSET_MM,
                })
...
RESULT_FIELDS = PLAN_FIELDS + [
    ...
    "hover_t105_dy_mm",
    "hover_t105_dz_mm",
    "hover_t105_xy_residual_mm",
    "hover_t105_3d_residual_mm",

    "measured_pointer_x_mm",
    "measured_pointer_y_mm",
...
def load_or_create_run(args, freeze_run, targets, predictions):
    if args.resume:
        ...
        if metadata.get("endpoint_protocol_id") != "HOVER_MANUAL_XY_V1":
            raise RuntimeError(
                "This resume directory was not created with the hover-measurement "
                "E4-A protocol. Do not mix contact/mark and hover runs."
            )

        return run_dir, plan, metadata

    run_id = now_id()
    run_dir = Path(args.out_root) / f"E4A_formal_hover_{args.camera}" / run_id
    ...
    metadata = {
        ...
        "endpoint_protocol_id": "HOVER_MANUAL_XY_V1",
        "camera": args.camera,
        "n_targets": 9,
        "methods": METHODS,
        "robot_repeats": 3,
        ...
        "endpoint_protocol": {
            "type": "manual hover endpoint measurement",
            "high_relative_to_pred_surface_mm": SAFE_HIGH_MM,
            "measure_hover_relative_to_pred_surface_mm": MEASURE_HOVER_OFFSET_MM,
            "hold_until_coordinate_confirmed": True,
            "physical_marking": False,
        },

        "primary_outcome": (
            "manually measured hover pointer XY error relative to known paper GT"
        ),
...
def get_hover_measurement(gt_x, gt_y, pred_x, pred_y):
    while True:
        print(
            "\nROBOT IS HOLDING POSITION. Read the pointer XY in the SAME paper_frame:"
        )
        print(f"  GT       = ({gt_x:+.2f}, {gt_y:+.2f}) mm")
        print(f"  predicted= ({pred_x:+.2f}, {pred_y:+.2f}) mm")
        print("Enter actual pointer XY as x,y  (example: -65.5,32.0)")
        print("Or type SKIP to flag this trial incomplete; Q to stop session.")

        s = input("hover_xy> ").strip()
        ...
        return {
            ...
            "measured_pointer_x_mm": mx,
            "measured_pointer_y_mm": my,
...
def normalize_plan_row(r):
    ...
    float_fields = [
        ...
        "robot_high_z_mm", "robot_measure_hover_z_mm",
    ]
...
def main():
    ...
    print("=" * 94)
    print("E4-A FORMAL HOVER ENDPOINT EXPERIMENT V2")
    print("=" * 94)
    ...
    print("  HIGH=surface+40 mm; MEASURE HOVER=surface+10 mm.")
    print()
    ...
    print("- pointer setup and paper measurement frame are unchanged.")
    ...
    try:
        ...
        for p in remaining:
            ...
            measure_z = p["robot_measure_hover_z_mm"]
            ...
            print(
                f"HIGH Z={high_z:.3f} | MEASURE HOVER Z={measure_z:.3f}"
            )
            print()
            print(
                "The robot will descend to the measurement hover and HOLD there. "
                "Measure/enter XY first; it will move away only after you confirm."
            )
            ...
            if h3 > HIGH_3D_RESIDUAL_GATE_MM:
                ...
                print("MEASURE HOVER WILL NOT BE COMMANDED.")
            ...
            # --------------------------------------------------------------
            # C. MEASURE HOVER — vertical only, then HOLD
            # --------------------------------------------------------------
            print("[3] Vertical descent to frozen MEASURE HOVER offset")
            move_t104(
                ser,
                rx, ry, measure_z,
                TARGET_TIT_RAD,
            ...
            time.sleep(MEASURE_WAIT_S)

            hover_state = fresh_t105(ser)
            mdx,mdy,mdz,mxy,m3 = pose_residual(
                hover_state,
                (rx, ry, measure_z),
            )

            print_pose(
                "[MEASURE HOVER — ROBOT WILL HOLD HERE]",
                hover_state,
                (rx, ry, measure_z),
            )

            # --------------------------------------------------------------
            # D. Manual XY measurement WHILE robot is still holding
            # --------------------------------------------------------------
            measurement = get_hover_measurement(
                gt_x, gt_y, pred_x, pred_y
            )

            # Whether measured, skipped, or stop requested, move away only NOW.
            print("[4] Measurement input finished -> vertical rise to HIGH")
            move_t104(
                ser,
                rx, ry, high_z,
                TARGET_TIT_RAD,
            ...
            if measurement["action"] == "STOP":
                print(
                    "Robot has moved safely away. This trial was executed but "
                    "no coordinate was saved, so this trial UID remains incomplete."
                )
                stop_requested = True
                break
            ...
            result.update({
                ...
                "hover_t105_x_mm": hover_state["x"],
                "hover_t105_y_mm": hover_state["y"],
                "hover_t105_z_mm": hover_state["z"],
                "hover_t105_dx_mm": mdx,
                "hover_t105_dy_mm": mdy,
                "hover_t105_dz_mm": mdz,
                "hover_t105_xy_residual_mm": mxy,
                "hover_t105_3d_residual_mm": m3,
            ...
            print(
                ...
                f"{summary['measured_trials']}/108 hover XY measurements"
            )

            if measurement["action"] == "SAVE_SKIP":
                print(
                    "[WARNING] This formal trial is incomplete because the "
                    "hover endpoint coordinate measurement was skipped."
                )
        ...
        if summary["by_method"]:
            print("\nCurrent measured hover E2E summary (do NOT interpret final until complete):")
            for method in METHODS:
